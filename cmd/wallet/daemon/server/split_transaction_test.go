...
import (
	...
	"github.com/ZorkNetwork/zorkmid/cmd/wallet/keys"
...
func TestEstimateComputeMassAfterSignatures(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		signedTxStep1Bytes, err := libwallet.Sign(params, mnemonics[:1], unsignedTransactionBytes, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		signedTxStep2Bytes, err := libwallet.Sign(params, mnemonics[1:2], signedTxStep1Bytes, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		extractedSignedTx, err := libwallet.ExtractTransaction(signedTxStep2Bytes, false)
		if err != nil {
...
func testEstimateMassIncreaseForSignaturesSetUp(t *testing.T, consensusConfig *consensus.Config) (
	*serialization.PartiallySignedTransaction, []string, *dagconfig.Params, func(keepDataDir bool)) {
	...
	unsignedTransaction, err := libwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
		[]*libwallet.Payment{{
			Address: address,
			Amount:  10,
		}}, selectedUTXOs)
