...
func (s *server) calculateFeeLimits(requestFeePolicy *pb.FeePolicy) (feeRate float64, maxFee uint64, err error) {
	...
	case *pb.FeePolicy_MaxFeeRate:
		...
		feeRate = min(estimate.Estimate.NormalBuckets[0].Feerate, requestFeePolicy.MaxFeeRate)
...
func (s *server) createUnsignedTransactions(address string, amount uint64, isSendAll bool, fromAddressesString []string, useExistingChangeAddress bool, requestFeePolicy *pb.FeePolicy) ([][]byte, error) {
	...
	unsignedTransaction, err := libwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) selectUTXOs(spendAmount uint64, isSendAll bool, feeRate float64, maxFee uint64, fromAddresses []*walletAddress) (
	selectedUTXOs []*libwallet.UTXO, totalReceived uint64, changeSompi uint64, err error) {
...
func (s *server) estimateFee(selectedUTXOs []*libwallet.UTXO, feeRate float64, maxFee uint64, recipientValue uint64) (uint64, error) {
	...
	var mockPayments []*libwallet.Payment
	if totalValue > recipientValue {
		mockPayments = []*libwallet.Payment{
			{
				Address: fakeAddr,
	...
	mockTx, err := libwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
...
func (s *server) estimateFeePerInput(feeRate float64) (uint64, error) {
	...
	mockTxWithoutUTXO, err := libwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, nil)
	if err != nil {
		return 0, err
