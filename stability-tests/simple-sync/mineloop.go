...
func mineLoop(syncerRPCClient, syncedRPCClient *rpc.Client) error {
	...
	for i := uint64(0); i < activeConfig().NumberOfBlocks; i++ {
		...
		if err != nil {
			...
			// TODO: Fix the race condition in miner so it won't panic (proper shutdown handler)
...
func mineBlock(syncerRPCAddress string, miningAddress util.Address) error {
	minerCmd, err := common.StartCmd("MINER",
		"miner",
	...
	return errors.Wrapf(minerCmd.Wait(), "error with command '%s'", minerCmd)
}
